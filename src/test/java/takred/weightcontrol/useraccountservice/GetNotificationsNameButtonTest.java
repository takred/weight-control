...
public class GetNotificationsNameButtonTest {
    @Mock
    ...
    void notificationsOn() {
        ...
        assertEquals("Выключить уведомления", userAccountService.getNotificationsNameButton(1));
    }
    ...
    void notificationsOff() {
        ...
        Optional<UserAccount> optionalUserAccount = Optional.of(userAccount);
        doReturn(optionalUserAccount).when(userAccountRepository).findById(1);
        assertEquals("Включить уведомления", userAccountService.getNotificationsNameButton(1));
    }
}
